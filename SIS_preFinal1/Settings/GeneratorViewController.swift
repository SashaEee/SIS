...
class GeneratorViewController: UIViewController, UITextViewDelegate {
    ...
    @IBOutlet weak var ViewSecure: UIView!
    ...
    override func viewDidLoad() {
        ...
        ScreenGuardManager.shared.guardScreenshot(for: ViewSecure)
        getFireBase()
        resetTimer()
        self.refreshQRCode()
        brightness()
...
//                            self.textUser = "А здесь небольшой секретик :)"
                            self.textView.text = self.textUser
                            print (self.textUser)
                            self.refreshQRCode()
                            self.hideActivityIndicator()
